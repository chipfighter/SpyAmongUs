...
负责管理系统中的消息流转、存储和广播功能，如下：
    1. 消息持久化：将用户和系统消息保存到Redis数据库
    2. 历史消息查询：获取房间的历史消息记录
    3. 实时消息广播：向房间内所有连接的用户发送消息
    4. WebSocket连接管理：维护房间与WebSocket连接的映射关系
    5. 系统消息生成与发送：创建并分发系统事件通知
